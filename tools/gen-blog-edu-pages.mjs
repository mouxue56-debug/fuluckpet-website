#!/usr/bin/env node
// Generate static /blog/{slug}.html for the 31 new edu articles.
// Adapted from gen-blog-static-pages.mjs.
...
${body}
  ...
  <a href="https://page.line.me/915hnnlk?oat__id=5765672&openQrModal=true" class="fixed-line" data-cta="line" aria-label="LINEでお問い合わせ" target="_blank" rel="noopener">
    ...
    <span class="fixed-line-content"><span class="fixed-line-label" data-i18n="header.telLabel">見学予約・ご相談</span><span class="fixed-line-cta" data-i18n="cta.line">LINEで相談</span></span>
  </a>
...
// Generate ONLY new articles (those with id starting art_edu_)
const merged = JSON.parse(fs.readFileSync('/tmp/blog-merged.json', 'utf-8'));
const newOnes = merged.filter(a => a.id && a.id.startsWith('art_edu_'));
console.log(`Generating ${newOnes.length} new static pages...`);
let total = 0;
for (const a of newOnes) {
  const dst = path.join(BLOG_DIR, `${a.slug}.html`);
  if (fs.existsSync(dst)) {
    console.log(`  SKIP (exists): ${a.slug}.html`);
    continue;
  }
...
console.log(`Done. ${total} new files written.`);
