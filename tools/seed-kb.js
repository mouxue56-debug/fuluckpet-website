...
function buildChunks(config = CONFIG) {
  ...
  'kb:visit':
    '見学は完全予約制で、希望日時は予約ページまたはLINEから伝える。現地見学に加え、LINEビデオ通話は事前相談・オンライン見学として利用できる。見学時間は約30分〜1時間。見学場所は大阪市城東区東中浜2-6-23（最寄り：Osaka Metro 緑橋駅）。契約前には登録事業所で子猫の現物確認と対面説明が必要で、ビデオ通話だけでは売買契約を成立させられない。お届け方法は空輸、関西圏の陸送、猫舎での直接お迎え。予約フォーム送信後はメールまたはLINEで連絡し、フォームが苦手な方はLINEから相談できる。',

  'kb:pricing':
    '子猫の料金は猫種・血統・カラーなどにより個体ごとに異なる。最新の金額は各子猫ページで確認するか、LINEで問い合わせる。表示価格は子猫本体価格（税込）で、3種混合ワクチン（1回目）の接種費用として別途10,000円（税込）を申し受ける。サイベリアン以外にもブリティッシュショートヘア・ブリティッシュロングヘアも取り扱う。ご成約時の予約金は50,000円で、残金は引き渡し日当日にお支払いいただく。過去にラグドールの繁育実績があるが、現在は繁育していない。',
