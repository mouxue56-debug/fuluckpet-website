...
const CHUNKS = {
  ...
  'kb:visit':
    '見学は完全予約制で、希望日時は予約ページまたはLINEから伝える。対面見学とLINEビデオ通話の両方に対応。見学時間は約30分〜1時間。ご家族での来訪も歓迎。場所は大阪府大阪市城東区東中浜で、詳細な住所は予約時に伝える。動物愛護管理法の規定により、購入前に必ず対面（来場またはLINEビデオ通話）での説明と現物確認が必要。お届け方法は3つ：空輸（全国対応、空港で受け取り、専用キャリー使用）、陸送（関西圏の大阪・兵庫・京都・奈良近郊）、直接お迎え（猫舎へ来場）。予約フォームはWebから送信可能で、24時間以内にメールまたはLINEで連絡。フォームが苦手な方はLINEから直接予約可能。',
