// admin-blog-editor.js — Rich-text blog editor controller for admin/blog-editor.html
...
(function() {
  ...
  // ===== State =====
  var articles = [];          // full list from /api/admin/articles
  var currentSlug = null;     // slug of article being edited (null = new)
  ...
  function renderArticleList() {
    ...
    if (filtered.length === 0) {
      list.innerHTML = '<div class="empty-list">' + tt('記事がありません', '暂无文章') + '</div>';
      return;
    }
    // Newest first
    ...
    list.innerHTML = filtered.map(function(a) {
      ...
      var when = (a.updatedAt || a.createdAt || '').slice(0, 10);
      ...
      return '<div class="article-item' + active + '" data-slug="' + escAttr(a.slug) + '">'
        ...
        +   '<div class="article-sub">' + escHtml(a.slug || '') + ' · ' + escHtml(when) + '</div>'
        + '</div>'
        + '</div>';
    }).join('');
  ...
  function loadArticleIntoEditor(slug) {
    ...
    if (!currentArticle.content) currentArticle.content = { ja: '', en: '', zh: '' };
    if (!currentArticle.tags) currentArticle.tags = [];
    fillFormFromArticle();
    renderArticleList();
    setDirty(false);
  }
  ...
  function fillFormFromArticle() {
    ...
    setEditorContent(a.content && a.content[currentLang] || '');
  ...
  function captureFormIntoArticle() {
    ...
    a.tags = ($('tagsInput').value || '').split(',').map(function(s) { return s.trim(); }).filter(Boolean);
    a.content[currentLang] = getEditorContent();
  }
  ...
  function saveArticle() {
    ...
    // POST to upsert endpoint (slug-keyed). Falls back to bulkImport if endpoint
    // not yet deployed.
    FuluckAPI.post('/api/admin/articles/upsert', a).then(function(saved) {
      ...
      // Fallback path: PUT then POST then bulk
      var idx = articles.findIndex(function(x) { return x.slug === a.slug; });
      var op;
      if (idx >= 0 && articles[idx].id) {
        op = FuluckAPI.put('/api/admin/articles/' + articles[idx].id, a);
      } else {
        op = FuluckAPI.post('/api/admin/articles', a);
      }
      op.then(function(saved) {
        ...
        showToast('Save error: ' + (err2.message || err2), 'error');
      });
    });
  }
  ...
  function deleteArticle() {
    if (!currentSlug) {
      showToast(tt('まず記事を選択してください', '请先选择文章'), 'error');
      return;
    }
    if (!confirm(tt('この記事を削除しますか？', '确定删除这篇文章吗？'))) return;
    FuluckAPI.del('/api/admin/articles/by-slug/' + encodeURIComponent(currentSlug)).then(function() {
      ...
      var a = articles.find(function(x) { return x.slug === currentSlug; });
      if (a && a.id) {
        FuluckAPI.del('/api/admin/articles/' + a.id).then(function() {
          showToast(tt('削除しました', '已删除'), 'success');
          currentSlug = null;
          currentArticle = null;
          newArticle();
          ...
          showToast('Delete error: ' + (err2.message || err2), 'error');
        });
      } else {
  ...
  function previewArticle() {
    ...
    var body = (a.content && a.content[lang]) || '';
  ...
  function bindAutoSlug() {
    ...
    ['excerptInput', 'tagsInput', 'publishedAt', 'coverImage'].forEach(function(id) {
      var el = $(id);
  ...
  function bindActions() {
    ...
    $('btnReload').addEventListener('click', function() { loadArticles(); });
