...
test('listing ItemList only links priced kittens with truthful detail Products', (t) => {
  ...
  const kittens = [
    ...
    { ...base, id: 'row-unpriced', breederId: 'schema-unpriced', status: 'available', price: '' },
  ];
  ...
  for (const lang of ['ja', 'en', 'zh']) {
    ...
    const itemList = listingItemList(fs.readFileSync(listPath, 'utf8'));
    assert.equal(itemList['@type'], 'ItemList');
    assert.equal(itemList.numberOfItems, 2, `${lang} schema must omit sold and unpriced inventory`);
    assert.equal(itemList.itemListElement.length, 2);
    assert.deepEqual(itemList.itemListElement.map((item) => item.position), [1, 2]);
    const serializedList = JSON.stringify(itemList);
    assert.doesNotMatch(serializedList, /schema-sold/);
    assert.doesNotMatch(serializedList, /schema-unpriced/);

    for (const breederId of ['schema-available', 'schema-reserved']) {
      const expectedUrl = `https://fuluckpet.com/${prefix}kittens/${breederId}.html`;
      const listItem = itemList.itemListElement.find((item) => item.url === expectedUrl);
      assert.ok(listItem, `${lang} ItemList must link ${breederId}`);
      const product = detailProduct(fs.readFileSync(path.join(siteDir, prefix, `kittens/${breederId}.html`), 'utf8'));
      assert.ok(product, `${lang} priced detail must publish one Product`);
      assert.equal(product['@id'], `https://fuluckpet.com/kittens/${breederId}.html#product`);
      assert.equal(product.offers.url, expectedUrl);
    ...
    assert.equal(
      detailProduct(fs.readFileSync(path.join(siteDir, prefix, 'kittens/schema-unpriced.html'), 'utf8')),
      null,
    );
  }
...
test('detail Product descriptions never invent individual health or temperament traits', (t) => {
  ...
  const descriptions = ['ja', 'en', 'zh'].map((lang) => {
    const prefix = lang === 'ja' ? '' : `${lang}/`;
    const product = detailProduct(fs.readFileSync(path.join(siteDir, prefix, 'kittens/schema-factual.html'), 'utf8'));
    assert.ok(product);
...
test('missing kitten prices never become zero-price offers or visible zero-yen claims', (t) => {
  ...
  for (const lang of ['ja', 'en', 'zh']) {
    ...
    assert.equal(itemList.numberOfItems, 1, `${lang} only the priced kitten can join the ItemList`);
    assert.equal(itemList.itemListElement.length, 1);
    assert.equal(itemList.itemListElement[0].url, `https://fuluckpet.com/${prefix}kittens/priced-kitten.html`);
    assert.doesNotMatch(listing, /(?:&yen;|¥)0\b/);
    assert.match(listing, new RegExp(copy[lang]));

    const pricedDetail = fs.readFileSync(path.join(siteDir, prefix, 'kittens/priced-kitten.html'), 'utf8');
    const product = detailProduct(pricedDetail);
    assert.ok(product, `${lang} priced detail must publish a Product`);
    assert.equal(product.offers.price, '180000');
    ...
    assert.equal(detailProduct(detail), null, 'unpriced detail cannot publish a zero-price Product');
    ...
    for (const id of ['boolean-price-kitten', 'array-price-kitten', 'object-price-kitten']) {
      ...
      assert.equal(detailProduct(coercedDetail), null, `${lang} ${id} cannot publish a coerced Product`);
