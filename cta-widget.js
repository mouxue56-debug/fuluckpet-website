...
(function() {
  ...
  if (!KittenCatalog) {
    console.warn('CTA widget: kitten-catalog.js must load before cta-widget.js');
    return;
  }
  ...
  getSharedKittens()
    .then(function(data) {
      kittenCount = KittenCatalog.orderKittens(data || []).filter(function(k) {
        return KittenCatalog.normalizeStatus(k.status) === 'available';
      }).length;
      render();
      checkScroll();
    })
